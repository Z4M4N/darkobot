...
local admin_num = "👤 تعداد مدیران ❇️: "..result.admins_count.."\n"
...
local function lock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'yes' then
    return '🔒کاراکتر های ویژه قفل میباشند📢'
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'yes'
    save_data(_config.moderation.data, data)
    return '🔐کاراکتر ویژه قفل شد📢'
  end
end
...
local function unlock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'no' then
    return '🔓کاراکتر ویژه باز میباشد📢'
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'no'
    save_data(_config.moderation.data, data)
    return '🔓کاراکتر ویژه باز شد📢'
  end
end
...
local function enable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'yes' then
    return '❇️تنظیمات امنیتی فعال است🔰'
  else
    data[tostring(target)]['settings']['strict'] = 'yes'
    save_data(_config.moderation.data, data)
    return '❇️تنظیمات امنیتی فعال شد🔰'
  end
end
...
local function disable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'no' then
    return '❌تنظیمات امنیتی غیرفعال است🔰'
  else
    data[tostring(target)]['settings']['strict'] = 'no'
    save_data(_config.moderation.data, data)
    return '❌تنظیمات امنیتی غیرفعال شد🔰'
  end
end
...
function get_message_callback(extra, success, result)
	...
	elseif get_cmd == "setadmin" then
		...
		if result.from.username then
			text = "@"..result.from.username.." مدیر شد❇️"
		else
			text = "[ "..user_id.." ] مدیر شد❇️"
		end
	...
	elseif get_cmd == "demoteadmin" then
		...
		if is_admin2(result.from.peer_id) then
			return send_large_msg(channel_id, "شما نمی توانید مدیران را بن گلوبالی کنید❇️")
		end
		channel_demote(channel_id, "user#id"..user_id, ok_cb, false)
		if result.from.username then
			text = "@"..result.from.username.." از مدیریت برکنار شد⛔️"
		else
			text = "[ "..user_id.." ] از مدیریت برکنار شد⛔️"
		end
	...
	elseif get_cmd == "setowner" then
		...
		local channel_id = 'channel#id'..result.to.peer_id
			...
			if result.from.username then
				text = "@"..result.from.username.." [ "..result.from.peer_id.." ] مدیر اصلی گروه شد✳️"
			else
				text = "[ "..result.from.peer_id.." ] مدیر اصلی گروه شد✳️"
			end
			send_large_msg(channel_id, text)
		end
	...
	elseif get_cmd == 'mute_user' then
		...
		elseif is_admin1(msg) then
			...
			send_large_msg(receiver, " ["..user_id.."] به لیست افراد بی صدا اضافه شد⛔️")
		end
	end
end
    ...
    if vusername == member or vpeer_id == memberid then
     ...
     local sender = cb_extra.msg.from.id
      ...
      if is_momod2(user_id, channel_id) and not is_admin2(sender) then
        return send_large_msg("channel#id"..channel_id, "⛔️شما نمی توانید مدیران را اخراج کنید📢")
      end
      if is_admin2(user_id) then
        return send_large_msg("channel#id"..channel_id, "⛔️شما نمی توانید بقیه مدیران را اخراج کنید📢")
      end
      if v.username then
        text = ""
...
elseif get_cmd == "setadmin" then
   for k,v in pairs(result) do
    ...
    if vusername == member or vpeer_id == memberid then
      ...
      channel_set_admin(channel_id, user_id, ok_cb, false)
      if v.username then
        text = "@"..v.username.." ["..v.peer_id.."] مدیر شد❇️"
        savelog(msg.to.id, name_log.." ["..msg.from.id.."] set admin @"..v.username.." ["..v.peer_id.."]")
      else
        text = "["..v.peer_id.."] مدیر شد❇️"
	...
	for k,v in pairs(result) do
		...
		if vusername == member or vpeer_id == memberid then
			...
			if group_owner then
				...
				if result.username then
					text = member_username.." ["..v.peer_id.."] مدیر اصلی گروه شد✳️"
				else
					text = "["..v.peer_id.."] مدیر اصلی گروه شد✳️"
				end
			end
...
local function set_supergroup_photo(msg, success, result)
  ...
  if success then
    ...
    send_large_msg(receiver, '⚠️خطا لطفا مجددا سعی نمایید⛔️', ok_cb, false)
  end
end
	...
	local data = load_data(_config.moderation.data)
		if matches[1] == 'add' and not matches[2] then
			...
			if is_super_group(msg) then
				return reply_msg(msg.id, '🌐این سوپرگروه در حال حاظر به لیست گروه های مدیریتی ربات اضافه شده است❇️', ok_cb, false)
			end
		...
		if matches[1] == "owner" then
			...
			if not group_owner then
				return "⚠️مدیر اصلی وجود ندارد لطفا با سودو صحبت کنید✅"
			end
		...
		if matches[1] == 'promote' then
		  if not is_momod(msg) then
			...
			if not is_owner(msg) then
				return "⚠️فقط مدیران می توانند شخصی را مدیر کنند📢"
			end
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'promote',
		...
		if matches[1] == 'demote' then
			...
			if not is_owner(msg) then
				return "⚠️فقط مدیران می توانند مدیری را اضافه کنند📢"
			end
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'demote',
		...
		if matches[1] == 'clean' then
			...
			if not is_momod(msg) then
				return "⚠️فقط مدیر اصلی می تواند تمام افراد را حذف کند📢"
			end
			if matches[2] == 'modlist' then
				if next(data[tostring(msg.to.id)]['moderators']) == nil then
					return '⚠️مدیری در این سوپرگروه وجود ندارد📢'
				end
				for k,v in pairs(data[tostring(msg.to.id)]['moderators']) do
